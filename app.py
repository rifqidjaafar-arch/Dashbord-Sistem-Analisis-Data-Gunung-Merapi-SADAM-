...
st.write(
    f"Library Siap Digunakan! | Last Check: {datetime.now().strftime('%H:%M:%S')}"
)

def ambil_data_merapi():
    # URL langsung ke daftar laporan
    ...
    try:
        ...
        # Mencari semua elemen kartu yang berisi informasi gunung
        # Di MAGMA v1, informasi biasanya dibungkus dalam class card atau col-md
        ...
        for card in cards:
            ...
            if "Merapi" in card_text:
                ...
                # Mengambil Pengamatan Visual (Biasanya di paragraf setelah judul)
        ...
        # Jika loop selesai dan tidak ketemu, gunakan data default agar dashboard tidak crash
        return {
...
# 1. Ambil data (asumsi fungsi ini sudah ada di environment Anda)
data = ambil_data_merapi()

# 2. Setup Data YouTube
...
# 4. Konversi peta ke HTML agar bisa disisipkan ke dalam dashboard
map_html = m._repr_html_()

# 5. Tampilkan Dashboard Gabungan
st.markdown(f"""
<div style="background-color:#f0f2f6; padding:30px; border-radius:15px; font-family:sans-serif; max-width:800px; margin:auto; box-shadow: 0 4px 15px rgba(0,0,0,0.1);">
    ...
    <!-- Section: Pengertian Gunung Merapi -->
